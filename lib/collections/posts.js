...
Posts.before.remove(function (userId, doc) {
  ...
  Posts.update(doc._id, {$set: {version: doc.version++}})
...
Posts.after.update(function (userId, doc) {
  ...
  Posts.update(doc._id, {$set: {version: doc.version++, modified: Date.now()}})
...
Posts.after.insert(function (userId, doc) {
  // change modified date
  Blogs.update(doc.blogId, {$set: {modified: Date.now()}});
  Posts.update(doc._id, {$set: {version: 1, modified: Date.now()}})
  var versionning = _.extend({postId: doc._id, modifiedBy: userId}, doc);
  PostsVersions.insert(versionning);
});
