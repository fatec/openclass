...
if(Meteor.isClient) {
  Counts = new Mongo.Collection("counts");// Subscribe to the count for the current room.
  //Meteor.subscribe("counts-by-room", Session.get('blogId'));
...
Posts.before.remove(function (userId, doc) { 
  ...
  // var image = Images.findOne({'metadata.postId': doc.imageId, 'metadata.last': true});
  // if (image){
  //   //console.log("On efface l'image "+image._id);
  //   Images.update(image._id, {$unset: {'metadata.unvalid': ""}, $set: {'metadata.last': false, 'metadata.modified': deletionTime}}, function(error, file){
  //     if (error) {
...
// On copie le post courant dans PostsVersions
...
Meteor.methods({
  ...
  addLikeComment: function(data) {
    ...
    console.log(data.currentPostId);
        console.log(data.currentCommentId);
                console.log(data.author);


    console.log(Posts.update({_id:data.currentPostId,"comments.id":data.currentCommentId}, {$push: {"comments.$.likes": data.author}}));
 ...
 //   console.log(Authors.findOne({blogId: postAttributes.blogId, name: 'Enseignant'}));
  //  check(postAttributes.body, String) || check(postAttributes.imageId, String)
    ...
    category = Categories.findOne({blogId: postAttributes.blogId, name: postAttributes.category});
    ...
    /*
    // On edite le blogPosts
    if (blog.posts == undefined) {
