        ...
        {params:{blog: blog, userDoingCopy: user}}, function (error,result) {
           if(error){
              ...
              if (result.data.params.error) {
                ...
                // localPosts:  Tous les posts sur le client
                var serverPosts = JSON.parse(result.data.params.result);
                var localPosts = Posts.find({blogId: blog._id}).fetch();
                ...
                var localPostsIds = [];
                var localPostsItem = 0;
                while (localPostsItem < localPosts.length) {
                  localPostsIds.push(localPosts[localPostsItem]._id);
                  localPostsItem++;
                }

                // ce qui est commun dans les 2 devra être vérifié plus en détails pour faire un update
                var intersectionIds = _.intersection(localPostsIds, serverPostsIds);
                ...
                var localPostsItem = 0;
                while (localPostsItem < localPosts.length) {
                  if (addToServerIds.indexOf(localPosts[localPostsItem]._id) > -1) {
                    postsToCopyOnServer.push(localPosts[localPostsItem]);
                    //console.log("Il faut ajouter au serveur: "+localPosts[localPostsItem].body);
                  }
                  localPostsItem++;
                }
                ...
                while (postItem < postsToCopyOnServer.length) {
                  console.log("CLIENT - Copy On Server :  "+postItem+": "+toCopyOnServer[postItem].body);
                  postItem++;
                };

                var postItem = 0;
                while (postItem < postsToCopyOnClient.length) {
                  console.log("CLIENT - Copy On Client :  "+postItem+": "+toCopyOnClient[postItem].body);
                  postItem++;
                };
              ...
              Meteor.http.call("POST",  serverIp+"/api/posts/",
                {params:{blogId: blog._id, blogTitle: blog.title,posts: postsToCopyOnServer, images: images, userDoingCopy: user}}, function (error,result) {
                  if (error) {
