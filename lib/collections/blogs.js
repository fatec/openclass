...
Meteor.methods({
    ...
    sendBlog: function(blogAttributes) {
      ...
      var result = Meteor.http.call("POST",  "http://129.194.30.228:3000/api/blogs/",
        {data:{blog: blog, userDoingCopy: user},headers:{"content-type":"application/json"}}, function (error,result) {
         if(error){
            console.log('Error '+error);
         }else{
            console.log("SEND BLOG : Résultat du POST "+result.data.data.result[2].lastEditionDate);
  //          console.log("SEND BLOG : Résultat du POST "+result.data.data.message);
            //return result;
         }
      });
...
// #############
    ...
    blogCopy: function(blogAttributes) {
      console.log("BlogCopy : On reçoit les informations suivantes: "+Object.keys(blogAttributes));
