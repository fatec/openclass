...
Blogs.before.remove(function (userId, doc) {
  // For each post -> delete
  var posts = Posts.find({blogId: doc._id}).fetch();

  // on efface le posts du blog (pour effacer correctement les images aussi..)
  var postsItem = 0;
  while (postsItem < posts.length) {
      Posts.remove(posts[postsItem]);
      postsItem++;
    }
  ...
  Meteor.call('addBlogVersion', versionning);
  });


Blogs.before.update(function (userId, doc) {
...
{
  var text = "";
  var possible = "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz123456789";
...
Meteor.methods({
    blogInsert: function(blogAttributes) {
        ...
        var code = makeCode(3);
        while (typeof AccessCodes.findOne({code: code}) != 'undefined')
          code = makeCode(3);

        AccessCodes.insert({code:code, userId:Meteor.userId()});
    ...
    sendBlog: function(blogAttributes) {
      ...
      var serverIp = "http://129.194.238.21:3000";
...
*/
                ...
                Meteor.http.call("POST",  serverIp+"/api/posts/",
                  {params:{blogId: blog._id, blogCode: blog.blogCode, blogTitle: blog.title,posts: postsToCopyOnServer, images: imagesToCopyOnServerString, postsToRemove: removeFromServerIds, userDoingCopy: user}}, function (error,result) {
                    if (error) {
    ...
    doesBlogExist: function(blogAttributes) {
      ...
      if (serverBlog == undefined) { // le blog n'existe pas encore
        ...
        if (owner == undefined) { // le propriétaire du blog sur le client n'existe pas ici
          ...
          var myBlog = Blogs.insert({_id: blog._id,title: blog.title, blogCode: blog.blogCode, userId: ownerId, author: blogAuthor,submitted: blog.submitted});
          ...
          if (blog.shortUrl !== undefined) {blog.shortUrl = Meteor.call('newShortUrl', blog._id, blog.shortUrl)}
          var myBlog = Blogs.insert({_id: blog._id,title: blog.title, blogCode: blog.blogCode, userId: ownerId, author: blogAuthor,submitted: blog.submitted, shortUrl: blog.shortUrl});
        }
        ...
        return {posts: serverPosts, images: serverImages, blogName: newBlogName, oldPosts: oldPosts};
