...
Meteor.methods({
        ...
        {params:{blog: blog, userDoingCopy: user}}, function (error,result) {
           if(error){
              ...
              if (result.data.params.error) {
                ...
                // On insère les records sur le client
                var imageItem = 0;
                while (imageItem < imagesToGetFromServer.length) {
                  // Si l'image existe deja on l'update (normalement ici il n'y a que de l'insert)
                  // TODO decommenter ici pour recupérer les meta-données de l'image    
                  //Images.upsert(imagesToGetFromServer[imageItem]._id, {$set: imagesToGetFromServer[imageItem]});
    ...
    doesBlogExist: function(blogAttributes) {
      ...
      if (serverBlog == undefined) {
        ...
        //var posts = Posts.find({blogId: blogAttributes.blogId}).fetch();
        var serverImages = Images.find({'metadata.blogId': blogAttributes.blogId}).fetch();
        serverImages = JSON.stringify(serverImages);
        //console.log("SERVER - doesBlogExist : Le blog existe déja.. on revoie tout les posts "+serverPosts);
        return {posts: serverPosts, images: serverImages};
