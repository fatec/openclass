...
Meteor.methods({
    ...
    sendBlog: function(blogAttributes) {
      ...
      console.log("Le blog avant qu'il soit envoyé "+blog._id);
        ...
        {params:{blog: blog, userDoingCopy: user}}, function (error,result) {
           if(error){
              ...
              if (result.data.params.result) {
                console.log("CLIENT : Il y a déjà un journal avec cet ID, voici le contenu des posts existant");
                // TODO.. debug here..
                var allPosts = JSON.parse(result.data.params.result);
                var postItem = 0;
                while (postItem < allPosts.length) {
                  console.log("CLIENT : Contenu du post "+postItem+": "+posts[postItem].body);
                  postItem++;
                };
              } else {
    ...
    doesBlogExist: function(blogAttributes) {
      ...
      console.log("doesBlogExist : blogId: "+blog._id);
      console.log("doesBlogExist : Titre du blog: "+blog.title);

      // On recherche s'il y a un blog qui correspond à cet id
      var blog = Blogs.findOne(blog._id);
      if (blog == undefined) {
        // le blog n'existe pas encore, alors on l'insert tel quel 
        // TODO decider que faire si le blog owner n'existe pas!
        var owner = Users.findOne(blog.userId);
        if (owner == undefined) {
          // Il faut ajouter le blog avec un owner qui est sur admin pour l'instant..
        } else {
          console.log("On insert le blog car il n'était pas présent");
          Blog.insert(blog);
        }
        return null;
      } else {
        // TODO : On renomme le blog s'il a changé de nom (en comparant modification date)
        var allPosts = Posts.find({blogId: blogAttributes.blogId}).fetch();
        allPosts = JSON.stringify(allPosts);
        console.log("SERVER : Le blog existe déja.. on revoie tout les posts "+allPosts);
        var result = [];
    ...
    blogCopy: function(blogAttributes) {
      ...
      console.log("BlogCopy : Infos recues: "+Object.keys(posts[0]));
      console.log("BlogCopy : Infos recues: "+posts[0].body);
      // Ici on recoit tous les posts du blog
      // pour chaque post on regarde s'il existe
      // si oui : On update le post
      // si non : On insert le post
      var postItem = 0;
      while (postItem < posts.length) {
        console.log("Contenu du post "+postItem+": "+posts[postItem].body);
        postItem++;
      };
