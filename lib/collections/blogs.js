...
Meteor.methods({
    ...
    blogCopy: function(blogAttributes) {
      ...
      while (imageItem < images.length) {
        ...
        // Si elle n'existe pas on le crée
          ...
          Mongo.Collection.get("cfs.images.filerecord").upsert(images[imageItem]._id, {$set: images[imageItem]}, function(error, idOfInsertedDocument){
              if (error) {
                console.log("SERVER - BlogCopy : error upsert in database "+error);
              } else {
                console.log("SERVER - BlogCopy : upsert in database SUCCESSFULL");
              }
          });
        postItem++;
      };
    ...
    imageCopy: function(blogAttributes) {
      ...
      var tempPath = process.env.PWD+"/.meteor/local/cfs/files/"+blogAttributes.store+"/temp-sync-"+blogAttributes.imageName;
      console.log("SERVER - imageCopy : chemin "+path);

      // On dépose l'image avec un nom temporaire.
      console.log("on depose l'image temporaire");
      base64_decode(blogAttributes.imageData, tempPath);
      console.log("-> image déposée");
      console.log("on fait un readstream avec cette image");
      // On cree un readstream avec cette image
      var readStream = fs.createReadStream(tempPath);
      console.log("-> readstream fait");

      console.log("on recherche l'image "+blogAttributes.imageName+" dans la base de donnée");
      // On récupère l'objet image
      var syncedImage = Images.findOne({'copies.images.key': blogAttributes.imageName});
      console.log("-> voila l'image "+syncedImage._id);
      console.log("on fait le write stream");
      // On cree le writestream de destination
      var writestream = syncedImage.createWriteStream()
      console.log("-> write stream fait");

      console.log("on pipe dedans");
