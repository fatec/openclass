...
// Blogs.before.insert(function (doc) {
//     doc.modified = Date.now();
//     console.log("ON INSERr");
// });

// Blogs.before.upsert(function (doc) {
//     doc.modified = Date.now();
...
*/
                ...
                // On envoit toutes les images du client vers le serveur TODO : envoyer seulement les nouvelles
                var image = 0;
                  ...
                  {params:{blogId: blog._id, blogCode: blog.blogCode, authors: clientAuthors, categories: clientCategories, tags: clientTags, blogTitle: blog.title,posts: postsToCopyOnServer, postsToRemove: removeFromServerIds, userDoingCopy: user}}, function (error,result) {
                    if (error) {
                      ...
                      // On envoi toutes les images TODO : seulement les nouvelles images
                        var images = Posts.find({blogId: blogAttributes.blogId,imageId:{$ne: false}}).forEach(function(post){
    ...
    doesBlogExist: function(blogAttributes) {
      ...
      // } else {
        ...
        if (serverBlog == undefined) { // le blog n'existe pas encore
          ...
          console.log(Posts.find({blogId: blog._id,imageId:{$ne: false}}).fetch());
          var serverImages = Posts.find({blogId: blog._id,imageId:{$ne: false},where:'cloud'}).fetch();// Workaround sync bug !!! (add where)
    ...
    imageCopy: function(imageAttributes) {
      ...
      if (!Posts.findOne({imageId:imageAttributes.imageName,where:'cloud'})) { // Workaround for sync bug
console.log("CEST JUSTE MON GARS");
