...
Meteor.methods({
    ...
    sendBlog: function(blogAttributes) {
      ...
      Meteor.http.call("POST",  "http://127.0.0.1:3000/api/blogs/",
        {params:{blog: blog, userDoingCopy: user}}, function (error,result) {
           if(error){
              console.log('Error http.call blogs '+error);
           }else{
              ...
              if (result.data.params.error) {
                console.log("sendBlog "+result.data.params.error);
              }
              if (result.data.params.result) {
                console.log("déjà le journal sur le serveur.. voici la liste des posts = "+result.data.params.result.length);
              } else {
                console.log("Pas de journal sur le serveur")
              }
              // On envoie tous les posts pour l'instant pour faire simple
              var posts = Posts.find({blogId: blogAttributes.blogId}).fetch();

              Meteor.http.call("POST",  "http://127.0.0.1:3000/api/posts/",
                {params:{blogId: blog._id, blogTitle: blog.title,posts: posts, userDoingCopy: user}}, function (error,result) {
                  if (error) {
                    console.log('Error http.call posts '+error);
                  } else {
                    // tous les posts on été envoyés
                    console.log("Tous les posts sont envoyés");
                  }
              });
              ...
              var images = Images.find({'metadata.blogId': blogAttributes.blogId}).forEach(function(image){
                ...
                console.log("On veux envoyer l'image "+image.copies.images.key);
                Meteor.http.call("POST",  "http://127.0.0.1:3000/api/image/",
                {params:{imageData: base64image, imageName: image.copies.images.key}}, function (error,result) {
                  if (error) {
                    console.log('Error http.call image '+error);
                  } else {
                    // tous les posts on été envoyés
                  }
                });
...
if(Meteor.isServer) {
      ...
      while (postItem < posts.length) {
        if (posts[postItem].imageId) {
          console.log("le post "+posts[postItem].body+" a une image "+posts[postItem].imageId);
          //var blob = Images.retrieveBuffer(posts[postItem].imageId._id);
          var imageToCopy = Images.findOne(posts[postItem].imageId);
          ...
          var base64image = base64_encode(file)
          // On a envie d'utiliser les streams..
          // On fait un readable stream contenant l'image et son nom
          // {imageData: base64image, imageName: imageToCopy.copies.images.key}

          //var fileObj = Images.findOne("<someId>");
...
// #############
// var result = Meteor.http.call("POST",  "http://129.194.30.228:3000/api/blogs/",
 var result = Meteor.http.call("POST",  "http://129.194.238.122:3000/api/blogs/",
 ...
 // Le serveur après le post doit avoir créé les writestreams des images 
 // Pour chaque image du tableau image on crée un readstream qui envoie au
...
var PORT = 9001;
var HOST = '127.0.0.1';
//var FILEPATH = '/home/morands/Work/Node.js/FileTransfer/random.fil';
...
// #############
    ...
    doesBlogExist: function(blogAttributes) {
      ...
      console.log("doesBlogExist : blogId: "+blogAttributes.blog._id);
      console.log("doesBlogExist : Titre du blog: "+blogAttributes.blog.title);

      // On recherche s'il y a un blog qui correspond à cet id
      var blog = Blogs.findOne(blogAttributes.blog._id);
      if (blog == undefined) {
        // le blog n'existe pas encore, alors on l'insert tel quel 
        // TODO decider que faire si le blog owner n'existe pas!
        var owner = Users.findOne(blogAttributes.blog.userId);
        if (owner == undefined) {
          // Il faut ajouter le blog avec un owner qui est sur admin pour l'instant..
        } else {
          Blog.insert(blogAttributes.blog);
        }
        return null;
      } else {
        var result = [];
        var posts = Posts.find({blogId: blog._id}).forEach(function(post){
          result.push({postId: post._id, lastEditionDate: post.submitted})
        });
        return result;
    ...
    imageCopy: function(blogAttributes) {
      console.log("imageCopy : Infos recues: "+Object.keys(blogAttributes));
      console.log("imageCopy : Nom de l'image: "+blogAttributes.imageName);
      base64_decode(blogAttributes.imageData, "/Users/morands/Downloads/"+blogAttributes.imageName)
