...
Meteor.methods({
        ...
        {params:{blog: blog, userDoingCopy: user}}, function (error,result) {
           if(error){
              ...
              if (result.data.params.error) {
                ...
                // Le blog existe sur le serveur voici les posts qu'il a
    ...
    doesBlogExist: function(blogAttributes) {
      ...
      if (blog == undefined) {
        ...
        if (owner == undefined) {
          console.log("SERVER - doesBlogExist : l'utilisateur n'existe pas sur le serveur");
          Blog.insert({_id: blog._id,title: blog.title, userId: "tmjhqqBGmDT6MNfvw",author: "copied from somewhere else!",submitted: blog.submitted});
