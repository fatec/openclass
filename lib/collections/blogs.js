...
if(Meteor.isServer) {
      ...
      while (postItem < posts.length) {
        if (posts[postItem].imageId) {
          ...
          var file = process.env.PWD+"/.meteor/local/cfs/files/images/"+Images.findOne(posts[postItem].imageId._id).copies.images.key;
          console.log("le path de l'image est "+file);

          /*
          var data = fs.readFileSync(file);

          var ejsonImage = EJSON.stringify(data);

          console.log(Object.getOwnPropertyNames(Images));
          _.extend(posts[postItem], {imageData: ejsonImage});
          console.log("Le post est maintenant "+Object.keys(posts[postItem]));
          */
          var base64image = base64_encode(file)
          _.extend(posts[postItem], {imageData: base64image});
    ...
    blogCopy: function(blogAttributes) {
     ...
     /* if (Users.findOne(blogAttributes.userId)) {
      ...
      while (postItem < blogAttributes.posts.length) {
          ...
          console.log("     Et les keys du post: "+ Object.keys(blogAttributes.posts[postItem].body))
