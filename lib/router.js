...
Router.route('/box', { name: 'boxAlert', yieldTemplates: {
    'header': {to: 'layout--header'},
    'boxAlert': {to: 'layout--main'}
        }});

Router.route('/data-protection', { name: 'dataProtection', yieldTemplates: {
    'headerBackButton': {to: 'layout--header'},
    'dataProtection': {to: 'layout--main'}
        }});
...
Router.route('/', { name: 'blogList', yieldTemplates: {
    'blogsHeader': {to: 'layout--header'},
    'blogList': {to: 'layout--main'}
        }, controller: 'BlogListController' });

Router.route('/journal/:_id', { name: 'blogPage', yieldTemplates: {
    'slideoutMenu': {to: 'layout--menu'},
...
Router.route('/api/posts', function(){
  console.log("On est dans l'API post");
  this.response.statusCode = 200;
  this.response.setHeader("Content-Type", "application/json");
  this.response.setHeader("Access-Control-Allow-Origin", "*");
  this.response.setHeader("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
